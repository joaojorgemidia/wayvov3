...
export function dbToMoto(r: any): Motorcycle {
  return {
    id: r.id, categoriaVeiculo: r.categoria_veiculo === "carro" ? "carro" : "moto",
    placa: r.placa || "", modelo: r.modelo || "", anoFabricacao: r.ano_fabricacao ?? null, anoModelo: r.ano_modelo ?? null,
    ...
    parceiroContato: r.parceiro_contato || undefined, status: r.status || "disponivel",
...
export function motoToDb(m: Motorcycle): any {
  return {
    categoria_veiculo: m.categoriaVeiculo || "moto",
    ...
    parceiro_contato: m.parceiroContato || null, status: m.status, km_atual: m.kmAtual,
...
export function dbToMaintenance(r: any): Maintenance {
  return {
    ...
    vincularLocatario: r.vincular_locatario ?? true,
    itens: r.itens || [],
...
export function maintenanceToDb(m: Maintenance): any {
  return {
    ...
    quem_paga: m.quemPaga || null, vincular_locatario: m.vincularLocatario ?? true, itens: m.itens || [],
...
export function dbToFinancial(r: any): FinancialEntry {
  return {
    ...
    dataOriginal: r.data_original || undefined,
    adiadoAte: r.adiado_ate || undefined,
